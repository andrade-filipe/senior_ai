...
> **Correção da correção (2026-04-19):** o item (2) acima está errado. `SseConnectionParams` existe em `google-adk==1.31.0` (`mcp_session_manager.py:89`) e é **a classe certa** para servidores FastMCP `transport="sse"`. `StreamableHTTPConnectionParams` fala outro protocolo e causou `HTTP 405` no E2E real. Snippet correto: `McpToolset(connection_params=SseConnectionParams(url=..., headers={"Accept": "application/json, text/event-stream", "X-Correlation-ID": cid}))`. Corrigido no template `agent.py.j2` e propagado para testes e snapshots. Detalhes em ADR-0001 § Correção da correção (2026-04-19) e `ai-context/references/DESIGN_AUDIT.md § C2`.
