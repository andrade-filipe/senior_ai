...
> **Correção da correção (2026-04-19):** o item (2) acima está **factualmente errado**. Inspeção direta do pacote `google-adk==1.31.0` mostra que `SseConnectionParams` existe e é pública em `google/adk/tools/mcp_tool/mcp_session_manager.py:89`, e o despacho em `mcp_session_manager.py:400` aponta para `sse_client()` (protocolo SSE legado: `GET /sse` + `POST /messages`). Já `StreamableHTTPConnectionParams` (linha 120) despacha para `streamablehttp_client()` em `mcp_session_manager.py:408` — protocolo **Streamable-HTTP** (POST num endpoint único), **incompatível** com um servidor FastMCP iniciado via `mcp.run(transport="sse")`. Na prática, o cliente Streamable-HTTP faz `POST /sse` e o servidor responde `HTTP 405 Method Not Allowed` (observado no run E2E de 2026-04-19). **A classe cliente correta para o nosso stack é `SseConnectionParams`.** Corrigido em `generated_agent/agent.py`, `transpiler/transpiler/templates/agent.py.j2`, fakes de teste e snapshots do transpilador. Guard de regressão em `tests/infra/test_mcp_transport_match.py`. A decisão-core desta ADR (transporte = SSE) continua válida; só a classe cliente foi corrigida. Detalhes em `ai-context/references/DESIGN_AUDIT.md § C2 nota de correção (2026-04-19)`.
