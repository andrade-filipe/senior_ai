...
O agente ADK consome via `MCPToolset(connection_params=SseConnectionParams(url="http://<service>:<port>/sse"))`.

Se, após a entrega, o MCP oficial marcar SSE como deprecado, abrimos ADR nova que supersede esta. Não migramos unilateralmente.
